...
const AccountCard = ({ account }) => {
  ...
  return (
    <>
      <div 
        className={'card  shadow-xl transition-all duration-200 hover:shadow-2xl hover:scale-102 hover:cursor-pointer'}
        onClick={handleClick}
      >
      ...
      <dialog className={`modal ${open ? 'modal-open' : ''}`}>
        <div className="modal-box w-11/12 max-w-7xl max-h-[90vh] overflow-y-auto">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-xl font-bold">{account.id}</h3>
            <button 
              onClick={handleClose}
